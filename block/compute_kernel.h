...
marshalParameterForCpuKernelCall(std::span<const T> *)
{
    ...
    auto convertParam = [] (std::span<const T> & out, ComputeKernelArgument & in, ComputeContext & context) -> Pin
    {
        if (in.getConstRange) {
            ...
            using namespace std;
            cerr << "converting span: ptr = " << ptr << " size = " << size << endl;
...
getAbstractType(MemoryArrayHandleT<T> *)
{
    ...
    GetConstRange getConstRange = [] (const std::any & val, ComputeContext & context) -> std::tuple<const void *, size_t, std::shared_ptr<const void>>
    {
        ...
        using namespace std;
        cerr << "getConstRange for memoryArrayHandle non-const" << endl;
...
struct ComputeContext {
    ...
    {
        using namespace std;
        cerr << "transferToCpu: array.handle = " << array.handle.get() << endl;
        ...
        cerr << "handle->data = " << handle->data << " handle->lengthInBytes = " << handle->lengthInBytes << endl;
