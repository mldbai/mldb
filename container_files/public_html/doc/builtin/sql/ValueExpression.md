...
If you handle the result of `try` as a row expression, then you should make
sure that both arguments supplied return row expressions.
...
will both still fail when an error is encoutered because then the function will
