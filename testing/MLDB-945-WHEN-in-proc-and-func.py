...
output = run_query_function(
    "timestamp() BETWEEN '2015-01-01' AND '2030-01-06'")
assert output['x'] == 9, 'expected row 9 value'

output = run_query_function(
    "timestamp() between '%s' and '%s'" % (now, in_two_hours))
assert not output, 'expected no output'

# SVD TRAIN FUNCTION
load_svd_dataset()
assert train_svd("timestamp() > '%s'" % in_two_hours, 1) == 2, \
    'expected 2 independent eigenvectors - when clause might have been applied incorrectly'
assert train_svd("timestamp() < '%s'" % in_two_hours, 2) == 3, \
