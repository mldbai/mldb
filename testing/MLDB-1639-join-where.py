...
class SampleTest(MldbUnitTest):
    ...
    def test_join_no_on_clause(self):
        ...
        res = mldb.query('select test1.x from (select \'toy story\' as x) as test1 join row_dataset({"toy story": 1, "terminator": 5}) as test2 where regex_search(test1.x, test2.column)')
        mldb.log(res)
    ...
    def test_join_outer_where(self):
        ...
        res = mldb.query('select * from (select \'toy story\' as x) as test1 right join row_dataset({"toy story": 1, "terminator": 5}) as test2 where test1.x = test2.column')
        mldb.log(res)
        ...
        res = mldb.query('select * from row_dataset({"toy story": 1, "terminator": 5}) as test2 left join (select \'toy story\' as x) as test1 where test1.x = test2.column')
        mldb.log(res)
    ...
    def test_join_outer_on(self):
       ...
       res = mldb.query('select * from (select \'everythingisawesome\' as x) as test1 right join row_dataset({"toy story": 1, "terminator": 5}) as test2 on test2.column = \'toy story\'')
       mldb.log(res)
       ...
       res = mldb.query('select * from row_dataset({"toy story": 1, "terminator": 5}) as test2 left join (select \'everythingisawesome\' as x) as test1 on test2.column = \'toy story\'')
       mldb.log(res)
