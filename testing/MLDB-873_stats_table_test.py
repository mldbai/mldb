...
class Mldb873Test(MldbUnitTest):
    ...
    def test_doc_example(self):
        ...
        dataset = mldb.create_dataset(dataset_config)
        now = datetime.datetime.now()
        dataset.record_row("br_1", [["host", "patate.com", now], 
                                    ["region", "qc", now], 
                                    ["click_on_ad", 1, now],
                                    ['purchase_value', 0, now]])
        dataset.record_row("br_2", [["host", "carotte.net", now], 
                                    ["region", "on", now], 
                                    ["click_on_ad", 1, now],
                                    ['purchase_value', 25, now]])
        dataset.record_row("br_3", [["host", "patate.com", now], 
                                    ["region", "on", now], 
                                    ["click_on_ad", 0, now],
                                    ['purchase_value', 12, now]])
        dataset.record_row("br_4", [["host", "carotte.net", now], 
                                    ["region", "on", now], 
                                    ["click_on_ad", 0, now],
                                    ['purchase_value', 0, now]])
        dataset.commit()
                           
        mldb.put("/v1/procedures/ad_stats", {
            "type": "statsTable.train",
            "params": {
                "trainingData": "SELECT * EXCLUDING(click_on_ad, purchase_value) FROM ad_data ORDER BY rowName()",
                "outputDataset": "ad_stats",
                "outcomes": [
                    ["click", "click_on_ad = 1"],
        ...
        self.assertTableResultEquals(mldb.query("SELECT * from ad_stats ORDER BY rowName()"), [
            [ "_rowName", "click.host", "click.region", "purchase.host", "purchase.region", "trial.host", "trial.region" ],
            [ "br_1", 0, 0, 0, 0, 0, 0],
            [ "br_2", 0, 0, 0, 0, 0, 0],
            [ "br_3", 1, 1, 0, 1, 1, 1],
            [ "br_4", 1, 1, 1, 1, 1, 2]
