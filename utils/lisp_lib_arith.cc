...
DEFINE_LISP_FUNCTION_COMPILER(plus, std, "+")
{
    ...
    std::vector<Pattern> patterns {
        Pattern::parse(context, "(+ $x:(type i64)) -> $x:(type i64)"),
        Pattern::parse(context, "(+ $x:(type u64)) -> $x:(type u64)"),
        Pattern::parse(context, "(+ $x:(type i64) $y:(type i64)) -> (`addi64 $x $y):(type i64)"),
        Pattern::parse(context, "(+ $x:(type i64) $y:(type i64)) -> (`addi64 $x $y):(type i64)"),
        Pattern::parse(context, "(+ $x:(type i64) $y:(type i64)) -> (`addi64 $x $y):(type i64)"),
        Pattern::parse(context, "(+ $x:(type i64) $y:(type u64)) -> (`addi64 $x (`tosigned64 $y)):(type i64)"),
        Pattern::parse(context, "(+ $x:(type u64) $y:(type i64)) -> (`addi64 (`tosigned64 $x) $y):(type i64)"),
        Pattern::parse(context, "(+ $x:(type u64) $y:(type u64)) -> (`addu64 $x $y):(type u64)"),
        Pattern::parse(context, "(+ $x:(type str) $y:(type str)) -> (`concat $x $y):(type str)"),
