...
struct ByteExtractorArray {
    ...
    constexpr T createMask() const { return numBytes == sizeof(T) ? -1 : (1ULL << (numBytes * 8)) - 1; }

    size_t numWordsToAllocate(size_t numElements) const
    {
        return (numBytes * numElements + sizeof(T) - 1) / sizeof(T) + 1; // +1 is because last entries read off end;
    }

    struct Writer {
        Writer(int numBytes, void * data)
            : numBytes(numBytes), data((char *)data)
        {
        }
        ...
        {
            T current;
            memmove(&current, data, sizeof(T));
            current |= val;
            memmove(data, &current, sizeof(T));
            //*data |= val;
            data += numBytes;
        }
    ...
    struct Reader {
        Reader(int numBytes, const void * data)
            : numBytes(numBytes), data((const char *)data),
