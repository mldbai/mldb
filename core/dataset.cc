...
getValue(uint32_t bucket) const
{
    if (bucket < numeric.offset)
        return 0;  // must be a null
    else if (bucket < strings.offset)
        return numeric.getValue(bucket);
    else if (bucket < blobs.offset)
        return blobs.getValue(bucket);
    else if (bucket < timestamps.offset)
        return timestamps.getValue(bucket);
    else return intervals.getValue(bucket);
}
#endif
...
{
    if (!active)
        throw HttpReturnException(500, "Attempt to get value from non-numeric bucket");
    if (bucket > splits.size())
        throw HttpReturnException(500, "Attempt to get out of range value from numeric bucket");
    if (bucket == splits.size()) // shouldn't really need this... TODO to figure it out
        return INFINITY;
}
#endif
...
initialize(std::vector<CellValue> values, int numBuckets)
{
    ...
    WritableBucketList buckets(totalRows, n);
        ...
        auto onValue = [&] (size_t, const CellValue & val)
            {
                uint32_t bucket;
                if (val.isNumeric())
                    bucket = numericValues[val];
                else bucket = categoricalValues[val];

                ExcAssertLess(bucket, n);
