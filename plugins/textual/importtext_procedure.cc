...
struct ImportTextProcedureWorkInstance {
                 ...
                 const std::function<bool (const Json::Value &)> & onProgress)
    {
        ...
        // If we have an empty line, store the line number here so that we can validate that
        // there is only one and it's the last line.
        std::atomic<int64_t> deferredEmptyLineNumber(-1);
                           ...
                           int64_t lineNum)
        {
            try {
                ...
                for (const char * p = lineStart;  allNewLines && p < lineStart + length;  ++p) {
                    char c = *p;
                    allNewLines = (c == '\n' || c == '\r');
                }
                //cerr << "allNewLines = " << allNewLines << endl;
                if (allNewLines) {
                    int64_t oldVal = -1;
                    if (!deferredEmptyLineNumber.compare_exchange_strong(oldVal, lineNum)) {
                        handleError("Empty line in file", actualLineNum, 0, string(line, length));
                    }
                    return true;
                }
