...
struct CaptureAudioSensor: public Sensor {
    ...
    CaptureAudioSensor(MldbEngine * engine,
        ...
        : Sensor(engine)
    {
        ...
        if (!ifmt) {
            ...
            throw AnnotatedException(500, "Cannot find alsa device");
        }
        ...
        if (!fmt_ctx) {
            ...
            throw AnnotatedException(500, "Cannot allocate context");
        }
        ...
        if (open_codec_context(&audio_stream_idx, fmt_ctx, AVMEDIA_TYPE_AUDIO) < 0) {
            throw AnnotatedException(500, "Couldn't open audio codec");
        }
    ...
    virtual ExpressionValue latest()
    {
        ...
        auto decodeOnePacked = [&] (const uint8_t * & start)
            {
                ...
                case AV_SAMPLE_FMT_S32P:        // signed 32 bits, planar
                    ...
                    start += 4;
                    break;
                case AV_SAMPLE_FMT_S64:         // signed 32 bits
                case AV_SAMPLE_FMT_S64P:        // signed 32 bits, planar
                ...
                case AV_SAMPLE_FMT_NB:
                    throw AnnotatedException(500, "Unknown audio sample format");
                }
                return result;
            };
#endif
        ...
        if (!frame) {
            ...
            ret = AVERROR(ENOMEM);
            throw AnnotatedException(600, "Couldn't allocate frame");
        }
        ...
        for (;;) {
            ...
            do {
                ...
                if (ret < 0) {
                    ...
                    fprintf(stderr, "Error decoding audio frame (%s)\n", errbuf);
                    cerr << "frame had " << pkt.size << " bytes" << endl;
                    throw AnnotatedException(400, "Error decoding audio frame");
                }
