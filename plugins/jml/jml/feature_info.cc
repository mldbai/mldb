...
unsigned Categorical_Info::parse(const std::string & name) const
{
    ...
    if (code == -1) {
        ...
        if (*name_end != 0 || code < 0 || code >= count()) {
            cerr << "code = " << code << " name = " << name << " count() = "
                 << count() << endl;
...
Feature_Info promote(const Feature_Info & i1, const Feature_Info & i2)
{
    ...
    if (i1.type() < 0 || (int)i1.type() >= 8 || i2.type() < 0 || (int)i2.type() >= 8)
...
print() const
{
    std::unique_lock<std::mutex> guard(lock, std::defer_lock);  // guard that doesn't acquire the lock
