...
struct IntegerFrozenColumn: public FrozenColumn {
    ...
    static size_t bytesRequired(const TabularDatasetColumn & column)
    {
        ...
        // For example, timestamps with a resolution of 1ms but only taken on
        // the whole second will always be a multiple of 1,000
...
/// Frozen column that stores each value as a number
struct NumberFrozenColumn: public FrozenColumn {
    NumberFrozenColumn(TabularDatasetColumn & column)
        : columnTypes(column.columnTypes)
    {
        firstEntry = column.minRowNumber;
        numEntries = column.maxRowNumber - column.minRowNumber + 1;
