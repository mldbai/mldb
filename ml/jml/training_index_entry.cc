...
void
Dataset_Index::Index_Entry::
initFiltered(const Index_Entry & other,
             ...
             int numMappedExamples)
{
    ...
    this->feature_space = other.feature_space;

    // All we take is examples and values from the other one

    if (other.examples.empty()) {
        // All examples are represented
        for (unsigned x = 0;  x < exampleMapping.size() && x < other.values.size();  ++x) {
            if (exampleMapping[x] == -1)
                continue;
            float value = other.values[x];
            insert(value, exampleMapping[x], numMappedExamples, false /* sparse */);
        }
    }
    else {
        // Only some examples are represented
        for (unsigned i = 0;  i < other.examples.size();  ++i) {
            unsigned x = other.examples[i];
            if (exampleMapping[x] == -1)
                continue;
            float value = other.values[i];
            insert(value, exampleMapping[x], numMappedExamples, true /* sparse */);
        }
    }
    
    finalize(numMappedExamples, other.feature, other.feature_space);
}
         ...
         std::shared_ptr<const Feature_Space> feature_space)
{
    ...
    if (dense() && exactly_one()) {
        ...
        // Examples may be empty if a feature never occcurred in this partition
        // of the data; so we can't assert on that
