...
public:
    ...
    /** Initialize by copying a subset of examples and features from
        another dataset.  This will also efficiently copy the
        indexes from the other dataset.

        The filtering on examples os done by the keepExamples parameter,
        which will normally be a distribution of weights.  For each
        one, if the weight is exactly equal to 0.0 then it will be
        filtered; otherwise it is kept.
    */
    void initFiltered(const Training_Data & other,
