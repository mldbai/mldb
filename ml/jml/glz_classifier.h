    ...
    */
    ...
    struct Feature_Spec {
        enum Type {
            ...
            VALUE_EQUALS        ///< True if the value equals the given value
        };

        Feature_Spec()
            : type(VALUE), value(std::numeric_limits<float>::quiet_NaN())
        {
        }
        ...
        Feature_Spec(const Feature & feature,
            ...
            : feature(feature), type(type), value(value)
        {
        }
        ...
        Type type;
        float value;
        std::string category;  ///< String value of category name
    };
