...
public:
    ...
    Mutable_Feature_Set(features_type feats, bool isSorted = false, bool locked = false)
        : features(std::move(feats)), is_sorted(isSorted), locked(locked)
    {
        do_sort();
    }
    ...
    Mutable_Feature_Set(Mutable_Feature_Set && other) noexcept = default;
    Mutable_Feature_Set(const Mutable_Feature_Set & other) = default;

    Mutable_Feature_Set & operator = (Mutable_Feature_Set && other) noexcept
    {
        swap(other);
        return *this;
    }

    void swap(Mutable_Feature_Set & other) noexcept
    {
...
inline void swap(Mutable_Feature_Set & fs1,
          Mutable_Feature_Set & fs2) noexcept
{
    fs1.swap(fs2);
}
