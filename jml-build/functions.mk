...
$$(BUILD_$(CWD)/$(2).lo_OBJ):	$$(tmpDIR)/$(CWD)/$(1) $(OBJ)/$(CWD)/.dir_exists $$(dir $$(OBJ)/$(CWD)/$(2))/.dir_exists
	...
	$$(if $(verbose_build),,@$$(BUILD_$(CWD)/$(2).lo_COMMAND2) -E -o /dev/stdout | wc > $$@.wc)
	$$(if $(verbose_build),,@echo "           $(COLOR_GREEN)     $(COLOR_RESET) $(COLOR_DARK_GRAY)`awk -f mldb/jml-build/print-timing.awk $$@.timing $$@.wc`$(COLOR_RESET)	$(CWD)/$(1)")
