...
# DOCKER_PUSH: if this is defined and the container have been successfully
# built, then the container will be pushed to the docker registry using the
# tags defined in $(DOCKER_TAGS)
# This should be added to local.mk, not edited here.
...
docker_%: altroot_prep_%  $(DOCKER_GLOBAL_DEPS) $(DOCKER_TARGET_DEPS)
	...
	@rm -f $(TMPBIN)/$(*).cid
	docker run --cidfile $(TMPBIN)/$(*).cid -v `pwd`:/tmp/build:ro $(DOCKER_BASE_IMAGE) sh /tmp/build/$(JML_BUILD)/docker_install_inside_container.sh /tmp/build/$(TMPBIN)/docker-$(*) $(if $(DOCKER_POST_INSTALL_SCRIPT),/tmp/build/$(DOCKER_POST_INSTALL_SCRIPT))
	cat $(TMPBIN)/$(*).cid
