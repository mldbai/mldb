...
auto visit(Visitor && visitor, JsonParsingContext & context) -> typename std::decay_t<Visitor>::return_type
{
    ...
    if (context.isNull()) {
        ...
        if constexpr (isVisitable<DecayedVisitor, Return(StructuredJsonParsingContext&, JsonArrayTag)>()) {
            return visitor.visit(JsonArrayTag{});
        }
    }
        ...
        if constexpr (isVisitable<DecayedVisitor, Return(StructuredJsonParsingContext&, JsonObjectTag)>()) {
            return visitor.visit(JsonObjectTag{});
        }
    }
...
template<typename Visitor, typename... ExtraTypes>
auto visit(Visitor && visitor, const Json::Value & value) -> typename std::decay_t<Visitor>::return_type
{
    StructuredJsonParsingContext context(value);
    return visit(std::forward<Visitor>(visitor), context);
}
